...
if (Meteor.isClient) {
	...
	UI.registerHelper('langPref', function(field, _lang) {
		var lang = _.isString(_lang) ? lang : undefined;
		return LangM1.langPref(this, field, lang);
	});
	UI.registerHelper('langUsing', function() {
		return LangM1.langUsing(this);
	});
	UI.registerHelper('langDir', function() {
		return dirFromLang(LangM1.langUsing(this));
	});



	UI.registerHelper('getLang', function(field, lang, noFallback) {
		if (!_.isString(lang)) lang = undefined;  // undo handlebars hash obj
		return getLang(this, field, lang, noFallback);
	});

	UI.registerHelper('dir', function(fromLang) {
		if (fromLang)
			return dirFromLang(fromLang);
		else
	...
	UI.registerHelper('langsButCurrent', function() {
	...
	function dirFromLang(lang) {
		return (jQuery.inArray(lang, ['he', 'ar']) == -1) ? 'ltr' : 'rtl'
	}
...
LangM1 = {
	...
	langUsing: function(data, lang) {
		// TODO, allow preferences
		if (!lang) lang = Session.get('lang');
		if (data.cuid == 6)
			console.log('want ' + lang);
		if (data.trans && data.trans[lang])
			return lang;
		return data.lang;
	},
	langPref: function(data, field, requestedLang) {
		var lang = this.langUsing(data, requestedLang);
		if (data.cuid == 6)
			console.log(lang, data.lang, data[field]);
		if (!lang || data.lang == lang)
			return data[field];
		else
			return data.trans[lang][field];
	}
};

LangM2 = {
	langUsing: function(data) {
		// TODO, allow preferences
		var lang = Session.get('lang');
	}
};
